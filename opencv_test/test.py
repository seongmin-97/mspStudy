...
def get_matcher():
    ...
    # 이 두친구는 바꿀 일이 없을 듯 match_conf는 속도랑 관련이 있는듯 함
    match_conf = 0.65 
    range_width = -1
...
def main():
    ...
    imgs, img_names = load_images_from_dir(args.dir)

    # opencv github에서 stitcher.create에서 봤던 그 부분인듯 상수 결정하는 부분
    work_megapix = 0.6
    seam_megapix = 0.1
    compose_megapix = -1
    conf_thresh = 1.0
    ...
    # 알고리즘 결정
    warp_type = WARP_CHOICES[3]
    blend_type = BLEND_CHOICES[0] # multiband
    blend_strength = 5
    ...
    for img in imgs:
        ...
        if work_megapix < 0:
            ...
            if is_work_scale_set is False:
                work_scale = min(1.0, np.sqrt(work_megapix * 1e6 / (img.shape[0] * img.shape[1])))
                is_work_scale_set = True
        ...
        re_img = cv.resize(src=img, dsize=None, fx=seam_scale, fy=seam_scale, interpolation=cv.INTER_LINEAR_EXACT)
    ...
    print(type(b), type(cameras))
    print(b, cameras)
    print('K : ', cameras[0].R, cameras[0].focal, cameras[0].aspect, cameras[0].ppx, cameras[0].ppy)
    ...
    # bundle adjustment, ray
    print('ba choices : ', list(BA_COST_CHOICES.keys())[0])
    ...
    # wave_correct 수평 수직 뭐 이런거
    if wave_correct is not None:
        rmats = []
        for cam in cameras:
            rmats.append(np.copy(cam.R))
        rmats = cv.detail.waveCorrect(rmats, cv.detail.WAVE_CORRECT_AUTO)
    ...
    seam_finder = SEAM_FIND_CHOICES['dp_color']
    ...
    for idx, name in enumerate(img_names):
        ...
        if blender is None and not timelapse:
            ...
            timelapser = cv.detail.Timelapser_createDefault(None)
