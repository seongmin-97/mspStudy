...
def computeKeypointsAndDescriptors(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    ...
    print("num of gaussian_kernels", gaussian_kernels.size)
...
def generateBaseImage(image, sigma, assumed_blur):
    ...
    return GaussianBlur(image, (0, 0), sigmaX=sigma, sigmaY=sigma)  # the image blur is now sigma instead of assumed_blur
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
    ...
    if all(center_pixel_value >= first_subimage) and \
        ...
        return True
    elif all(center_pixel_value <= first_subimage) and \
               all(center_pixel_value <= third_subimage) and \
               all(center_pixel_value <= second_subimage[0, :]) and \
               all(center_pixel_value <= second_subimage[2, :]) and \
               center_pixel_value <= second_subimage[1, 0] and \
